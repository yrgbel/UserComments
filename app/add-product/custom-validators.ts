...
export function categoryValidator(control: AbstractControl): { [key: string]: any } {

    let errorValue = control.value;
    let value: string = (control.value as string).toUpperCase();

    if (value !== "ALL PRODUCTS") {
        return null;
    } else {
        return { "categoryValidator": { errorValue } }
    }
}
